...
export function VisualTimerSettings() {
  const { visualTimerEnabled, setVisualTimerEnabled } = useTimerSettingsContext();

  return (
    <motion.button
      onClick={() => setVisualTimerEnabled(!visualTimerEnabled)}
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="text-left">
          ...
          <p className="text-xs text-tertiary light:text-tertiary-dark">
            Show progress ring around countdown
          </p>
        </div>
      </div>
      {/* Toggle Switch */}
      ...
      >
        <motion.div
          ...
          animate={{ left: visualTimerEnabled ? 20 : 4 }}
          transition={{ type: 'spring', ...SPRING.default }}
        />
      </div>
    </motion.button>
